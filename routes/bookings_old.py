...
# ─────────────────────────────────────────────────────────────────────────────
#  EDIT RITUAL DETAILS  (admin can change ritual/package on a booking) - Subject to later additions
# ─────────────────────────────────────────────────────────────────────────────
 
# @bookings_bp.route('/<int:booking_id>/edit-ritual', methods=['POST'])
# def edit_ritual_details(booking_id):
#     booking = Booking.query.get_or_404(booking_id)
 
#     if booking.status in [BookingStatus.CANCELLED.value, BookingStatus.COMPLETED.value]:
#         flash('Cannot edit ritual details for a cancelled or completed booking.', 'error')
...
#     if not ritual_package_id:
#         flash('Please select a ritual package.', 'error')
#         return redirect(url_for('bookings.view_booking', booking_id=booking_id))
 
#     if not edit_reason:
#         flash('Please provide a reason for the ritual change.', 'error')
#         return redirect(url_for('bookings.view_booking', booking_id=booking_id))
 
#     package = RitualPackage.query.get(ritual_package_id)
#     if not package:
#         flash('Invalid ritual package selected.', 'error')
#         return redirect(url_for('bookings.view_booking', booking_id=booking_id))
 
#     old_title = booking.ritual_type
#     new_price = package.price
 
#     # Update booking fields
#     booking.ritual_package_id = package.id
#     booking.ritual_snapshot   = _build_ritual_snapshot(package)
#     # Adjust amount_due by the price difference
#     price_diff         = new_price - booking.total_amount
#     booking.total_amount += price_diff
#     booking.amount_due   = max(0, booking.amount_due + price_diff)
#     booking.reschedule_reason = (
#         f'[RITUAL EDIT] Changed from "{old_title}" to '
#         f'"{package.ritual_title} — {package.package_type}". Reason: {edit_reason}'
#     )
#     booking.updated_at = datetime.utcnow()
 
#     # Keep checkout_item in sync if it exists
#     if booking.checkout_item:
#         booking.checkout_item.ritual_package_id     = package.id
#         booking.checkout_item.ritual_title          = package.ritual_title
#         booking.checkout_item.package_type          = package.package_type
#         booking.checkout_item.price                 = new_price
#         booking.checkout_item.other_snapshot_fields = {
#             'description':  package.description or '',
#             'included':     _get_pkg_included(package),
#             'not_included': _get_pkg_not_included(package),
#             'token_amount': package.token_amount,
#         }
#         booking.checkout_item.updated_at = datetime.utcnow()
 
#     db.session.commit()
#     flash(
#         f'Ritual details updated to "{package.ritual_title} — {package.package_type}" '
#         f'(₹{new_price:,.0f}).', 'success'
#     )
#     return redirect(url_for('bookings.view_booking', booking_id=booking_id))
...
#     booking.updated_at          = datetime.utcnow()
#     if booking.checkout_item:
#         booking.checkout_item.status = BookingStatus.CANCELLED.value #CheckoutStatus.STARTED.value
#     db.session.commit()
#     flash('Booking cancelled.', 'warning')
#     return redirect(url_for('bookings.view_booking', booking_id=booking_id))
